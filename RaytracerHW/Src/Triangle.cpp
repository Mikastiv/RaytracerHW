...
{
    // const auto normal = Cross((mV3 - mV1), (mV2 - mV1)).Normalize();
    // const float t = (Dot(mV1, normal) - Dot(ray.mOrigin, normal)) / Dot(ray.mDir, normal);
    // const auto point = ray.mOrigin + (ray.mDir * t);
    ...
    if (a > -Config::Epsilon && a < Config::Epsilon)
        return {};
    ...
    const float v = Dot(ray.mDir, q) * f;
